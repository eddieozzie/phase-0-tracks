...
#presents the original rating of each movie
movies.each do |movie, rating|
	puts "The movie #{movie.to_s} has a rating of #{rating}"
end

#presents the original actor list
actors.each do |actor|
	puts "The actor #{actor} has an index of #{actors.index(actor)}"
end

#jumbles the actors' names
actors.map! do |actor|
	actor.reverse
end

#presents the final rating of each movie
movies.each do |movie, rating|
	puts "The movie #{movie.to_s} has a rating of #{rating}"
end

#presents the changed actor list
actors.each do |actor|
	puts "The actor #{actor} has an index of #{actors.index(actor)}"
end

# Release 2
# method iterates through items, deleting any that meets certain condition
...
# method filters a data structure for items to meet condition
p grocery_list.select { |item| item.length == 5 }

p dinner_menu.select { |course, meal| meal.length == 5 }

# filters data structure, different method for meeting condition
